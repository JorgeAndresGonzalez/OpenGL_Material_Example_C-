...
void setModelMat(Shader s, vec3 vc3Pos, vec3 Scale,int i)
{
    ...
    if(i)
        modelMat = rotate(modelMat,(float)glfwGetTime(),vec3(0.0f,1.0f,0.0f));
...
int main ()
{
    ...
    shader.setVec3("light.diffuse",  vec3(0.5f, 0.5f, 0.5f));
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        for(int i=0; i<10; i++)
        {
            ...
            ///Set the material of the cubes (How much color it reflects from
            /// each type of light)
            shader.setFloat("material.shine", 16.0f);
            shader.setVec3("material.ambientStrength", vec3(1.0f, 0.5f, 0.31f));
            shader.setVec3("material.diffuseStrength", vec3(1.0f, 0.5f, 0.31f));
            shader.setVec3("material.specularStrength",vec3(1.0f, 0.5f, 0.31f));
